			...
			<form action="" id="manage_user">
				...
				<div class="row">
					<div class="col-md-6 border-right">
						...
						<div class="form-group">
							...
							<select name="type" id="type" class="custom-select custom-select-sm">
								<option value="3" <?php echo isset($type) && $type == 3 ? 'selected' : '' ?>>Comptabilité</option>
								<option value="2" <?php echo isset($type) && $type == 2 ? 'selected' : '' ?>>Opération</option>
					...
					<div class="col-md-6">
						...
						<div class="form-group">
							<label class="label control-label">Confirmation du mot de passe</label>
...
<script>
	$('[name="password"],[name="cpass"]').keyup(function(){
		...
		if(cpass == '' ||pass == ''){
			...
			if(cpass == pass){
				$('#pass_match').attr('data-status','1').html('<i class="text-success">Le mot de passe correspond.</i>')
			}else{
		...
		$.ajax({
			...
			success:function(resp){
				if(resp == 1){
					...
					setTimeout(function(){
						location.replace('index.php?page=liste_du_personnel')
					},750)
				}else if(resp == 2){
					$('#msg').html("<div class='alert alert-danger'>Email déjà existant.</div>");
