<style>
th {
  color: #007bff;
}
</style>

<?php include 'db_connect.php' ?>
<div class="col-lg-12">
	<div class="card card-outline card-primary">
		<div class="card-header">
			<div class="card-tools">
				<a class="btn btn-block btn-sm btn-default btn-flat border-primary " href="./index.php?page=nouvelle_facture"><i class="fa fa-plus"></i> Ajouter nouvelle facture</a>
			</div>
		</div>
		<div class="card-body">
			<table class="table tabe-hover table-bordered" id="list">
				<thead>
					<tr>
						<th class="text-center">#</th>
						<th class="text-center">Facture</th>
						<th class="text-center">Numéro Conteneur</th>
						<th class="text-center">Date Facturation</th>
						<th class="text-center">Expéditeur</th>
						<th class="text-center">Montant (Euro)</th>
						<!--<th class="text-center">Reçu (Euro)</th>-->
						<th class="text-center">créance</th>
						<th class="text-center">Status</th>
						<th class="text-center">Actions</th>
					</tr>
				</thead>
				<tbody>
					<?php
					$i = 1;
					$num_conteneur = $conn->query("SELECT * FROM conteneur ");
					$design_arr[0]= "Unset";
					while($row=$num_conteneur->fetch_assoc()){
						$design_arr[$row['id']] =$row['nom_c'];
					}
					$where = "";
					if(isset($_GET['s'])){
						$where = " where status = {$_GET['s']} ";
					}
					//SUM(paiement.montant) as total
                    $qry = $conn->query("SELECT sid, invoice_no, sender_name, grand_total, status, 
					date_depot, num_conteneur, creance
					FROM invoice $where");
					while($row= $qry->fetch_assoc()):
					?>
					<tr>
						<td class="text-center"><?php echo $i++ ?></td>
						<td class="text-center"><b><?php echo ucwords($row['invoice_no']) ?></b></td>
						<td class="text-center"><b><?php echo isset($design_arr[$row['num_conteneur']]) ? $design_arr[$row['num_conteneur']] : 'Unknow Designation' ?></b></td>
						<td class="text-center"><b><?php echo date("d-M-Y",strtotime($row['date_depot'])) ?></b></td>
						<td class="text-center"><b><?php echo ucwords($row['sender_name']) ?></b></td>
                        <td class="text-center"><b><?php echo ucwords($row['grand_total']) ?></b></td>
						<!--<td class="text-center"><b><?php echo ucwords($row['montant']) ?></b></td>-->
						<td class="text-center"><b><?php echo ucwords($row['creance']) ?></b></td>
						<td class="text-center">
							<?php 
							switch ($row['status']) {
								case '1':
									echo "<span class='badge badge-pill badge-info'> En cours</span>";
									break;
								case '2':
									echo "<span class='badge badge-pill badge-success'> Payé</span>";
									break;
								
								default:
									echo "<span class='badge badge-pill badge-danger'> Impayé</span>";
									
									break;
							}

							?>
						</td>
						<td class="text-center">
		                    <div class="btn-group">
								<?php if($_SESSION['login_type'] != 3): ?>
									<a href="index.php?page=modif_facture&id=<?php echo $row['sid'] ?>" class="btn btn-primary btn-flat ">
									<i class="fas fa-edit"></i>
									</a>
								<?php endif; ?>
                                <a href=" print.php?&id=<?php echo $row['sid'] ?>" target="_blank" class="btn btn-primary btn-flat ">
		                          <i class="fas fa-print"></i>
		                        </a>
								<?php 
								if ($row['status'] == 2){
								?>

								<?php
								}else{
									?>
									<?php if($_SESSION['login_type'] != 2): ?>
								<a href="index.php?page=paiement&id=<?php echo $row['sid'] ?>" class="btn btn-primary btn-flat ">
									<i class="fas fa-money-bill"></i>
								</a>
								<?php endif; ?>
								<?php
								}
								?>
								<?php if($_SESSION['login_type'] != 3): ?>
									<button type="button" class="btn btn-danger btn-flat delete_facture" data-id="<?php echo $row['sid'] ?>">
									<i class="fas fa-trash"></i>
									</button>
								  <?php endif; ?>
	                        </div>
						</td>
					</tr>	
					<?php endwhile; ?>
				</tbody>
			</table>
		</div>
	</div>
</div>
<style>
	table td{
		vertical-align: middle !important;
	}
</style>
<script>
	$(document).ready(function(){
		$('#list').dataTable()
		$('.view_parcel').click(function(){
			uni_modal("Parcel's Details","view_parcel.php?id="+$(this).attr('data-id'),"large")
		})
	$('.delete_facture').click(function(){
	_conf("Êtes-vous sûr de vouloir supprimer cette facture?","delete_facture",[$(this).attr('data-id')])
	})
	})
	function delete_facture($id){
		start_load()
		$.ajax({
			url:'ajax.php?action=delete_facture',
			method:'POST',
			data:{id:$id},
			success:function(resp){
				if(resp==1){
					alert_toast("Données supprimées avec succès",'success')
					setTimeout(function(){
						location.reload()
					},1500)

				}
			}
		})
	}
</script>